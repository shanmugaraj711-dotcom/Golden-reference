...
def _headers(request,content_type):
    request.send_header("Content-Type",content_type)
    request.send_header("Cache-Control","no-store")
    for k,v in SECURITY_HEADERS.items(): request.send_header(k,v)

def send_json(request,status,payload):
    body=json.dumps(payload).encode("utf-8")
    request.send_response(status); _headers(request,"application/json; charset=utf-8"); request.send_header("Content-Length",str(len(body))); request.end_headers(); request.wfile.write(body)

def send_html(request,html):
    body=html.encode("utf-8"); request.send_response(200); _headers(request,"text/html; charset=utf-8"); request.send_header("Content-Security-Policy","default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; base-uri 'none'; frame-ancestors 'none'"); request.send_header("Content-Length",str(len(body))); request.end_headers(); request.wfile.write(body)
    ...
    def do_GET(self):
        ...
        if path=="/api/projects":
            try:
                ...
                send_json(self,200,{"status":"ok","project":normalise(p),"lifecycleStates":STATES})
            except Exception as e: send_json(self,503 if "firebase" in str(e).lower() else 500,{"status":"error","error":"service unavailable"})
            return
    ...
    def do_POST(self):
        ...
        if path=="/api/projects":
            try:
                ...
                if length<0 or length>MAX_BODY_BYTES: send_json(self,413,{"status":"request_too_large","error":"request body exceeds limit"}); return
            ...
            except json.JSONDecodeError: send_json(self,400,{"status":"invalid_request","error":"invalid JSON"})
            ...
            except Exception: send_json(self,503,{"status":"service_unavailable","error":"service unavailable"})
            return
        send_json(self,404,{"status":"not_found","path":path})
    def do_PUT(self): send_json(self,405,{"status":"method_not_allowed","error":"method not allowed"})
    def do_PATCH(self): send_json(self,405,{"status":"method_not_allowed","error":"method not allowed"})
    def do_DELETE(self): send_json(self,405,{"status":"method_not_allowed","error":"method not allowed"})
